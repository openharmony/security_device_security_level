...
static int32 Invoke(IServerProxy *iProxy, int funcId, void *origin, IpcIo *req, IpcIo *reply)
{
    DslmFeatureApi *api = (DslmFeatureApi *)iProxy;
    switch (funcId) {
        case CMD_SET_DEVICE_SECURITY_LEVEL:
            return api->DslmGetDeviceSecurityLevel((IUnknown *)iProxy, req, reply);
        default:
            WriteInt32(reply, ERR_REQUEST_CODE_ERR);
            break;
    }
